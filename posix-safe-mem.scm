(module posix-safe-mem
        *
        ...
        (define-record safe-mem 
          semaphore semaphore-name memory-size shared-memory shared-memory-path memory-map copy)
        ...
        (define-syntax with-safe-mem
          (syntax-rules ()
            ((with-safe-mem () body1 body2 ...)
             (begin body1 body2 ...))
            ;; ((with-safe-mem safe-mem body1 body2 ...)
            ;;  (with-safe-mem (safe-mem) body1 body2 ...))
            ((with-safe-mem (safe-mem1 safe-mem2 ...) body1 body2 ...)
             (dynamic-wind
                 (lambda ()
                   (sem-wait (safe-mem-semaphore safe-mem1)))
                 (lambda ()
                   (with-safe-mem (safe-mem2 ...) body1 body2 ...))
                 (lambda ()
                   (sem-post (safe-mem-semaphore safe-mem1)))))))
        ...
        (define make-safe-mem
          (let ((%make make-safe-mem))
            (lambda (value 
                     #!key (semaphore-name (string-concatenate (list "/safe-mem-semaphore-" (uuid-v4))))
                     (shared-memory-name (string-concatenate (list "/safe-mem-shared-memory-" (uuid-v4))))
